...
/**
    ...
    и роль, которая была ему выдана. По умолчанию равна "Пользователь".
    Является константой.
 */
contract UsersContract is MainAdminRole, AdminRole, MailWorkerRole, Ownable {
    ...
    constructor () public {
        ...
        usersCount++;
        usersInfo(_owner, sha256(abi.encode('admin')), 'admin', 'admin', 'admin', 'Санкт-Петербург, ул. Мира, д. 6');
        users[usersCount] = usersInfo(_owner, '', 'admin', 'admin', 'admin', 'Санкт-Петербург, ул. Мира, д. 6');
        usersByAddress[_owner] = usersInfo(_owner, '', 'admin', 'admin', 'admin', 'Санкт-Петербург, ул. Мира, д. 6');
    }
    /**
     ...
     *   потом взаимодействовать с сайтом. Также присваивает зарегистрированным пользователям роль "Пользователь"
     *   @return bool
     */
    function _register(address _account, string memory _password, string memory _name, string memory _secondName, string memory _patronym, string memory _homeAddr) private returns (bool success){
        usersCount++;
        balancesAll[_account] = _account.balance;
        users[usersCount] = usersInfo(_account, sha256(abi.encodePacked(_password)), _name, _secondName, _patronym, _homeAddr);
        usersByAddress[_account] = usersInfo(_account, sha256(abi.encodePacked(_password)), _name, _secondName, _patronym, _homeAddr);
    ...
    function registration(address _account, string memory _password, string memory name, string memory secondName, string memory patronym, string memory _homeAddr)
        public
